        ...
        float sample_stride)
{
    ...
    grid_idxs[pt_index*3] = floor(x/sample_stride);
    grid_idxs[pt_index*3+1] = floor(y/sample_stride);
    grid_idxs[pt_index*3+2] = floor(z/sample_stride);
...
)
{
    ...
    gridDownSampleKernel<<<block_dim,thread_dim>>>(
            d_pts,d_grid_idxs,pt_num,pt_stride,sample_stride
    );
